...
static void	one_chunk(t_sha384 *sha384_info)
{
	int			i;
	uint64_t	w[80];
	uint64_t	state[8];
	...
	while (++i < 16)
	{
		sha384_info->chunk.block[i] =
		convert_endian_64bits(sha384_info->chunk.block[i]);
	...
	i = -1;
	while (++i < 8)
		sha384_info->state[i] = state[i] + sha384_info->state[i];
}

t_string	*crypto_algo_sha384(t_output *output_handle, char *args)
{
	t_sha384	sha384;
	int			padded;
	size_t		bytes_copied;
	...
	while (bytes_copied >= 128 - 16)
	{
		...
		fill_chunk_e(sha384.digest->string, &sha384.chunk, 0, &padded);
		one_chunk(&sha384);
		make_digest_sha384(sha384.state, sha384.digest);
		bytes_copied = request_chunk(output_handle, sha384.digest, 128);
	}
	ft_bzero(&sha384.chunk, sizeof(sha384.chunk));
	fill_chunk_e(sha384.digest->string, &sha384.chunk, 1, &padded);
	sha384.chunk.block[15] = convert_endian_64bits(output_handle->at * 8);
	one_chunk(&sha384);
	make_digest_sha384(sha384.state, sha384.digest);
	sha384.digest->length = 128 - 32;
